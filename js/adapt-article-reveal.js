...
define(function(require) {
    ...
    var ArticleRevealView = Backbone.View.extend({
        ...
        setDeviceSize: function() {
            var setToDesktop = (Adapt.device.screenSize === 'large' || Adapt.device.screenSize === 'medium');
            if (setToDesktop) {
        ...
        revealArticle: function(event) {
            if (event) event.preventDefault();
            if(this.$el.closest(".article").hasClass("locked")) {
                return; // in conjunction with pageLocking
            }
